...
This repository is the home of the **free trial** for Windows. It is not the source code — fontART is the one tool in this workshop that is not open source, and the section [Rights](#rights) says why.

![Windows](https://img.shields.io/badge/Windows-10%20%2F%2011-a21caf) ![Trial](https://img.shields.io/badge/Trial-60%20minutes%20per%20start-c026d3) ![Export](https://img.shields.io/badge/Export-a%E2%80%93z%20only-e879f9) ![Portable](https://img.shields.io/badge/Portable-no%20installation-eac54f)

**→ [Download the latest version](https://github.com/Dennismit2n/fontART-demo/releases/latest)** · [Deutsche Fassung dieser Datei](README.de.md) · [Guide and news](https://dennismit2n.github.io/werkstatt.html#fontart)

---

## Before you download: Windows will warn you
...
| ⏳ **60 minutes per start** | The clock starts with the program and cannot be paused; a minimised window keeps counting. When time is up, drawing and exporting stop. Saving keeps working, so you never lose your drawing. |
...
1. **[Download](https://github.com/Dennismit2n/fontART-demo/releases/latest)** the `.exe` and start it. Nothing is installed; the file runs as it is, and it can be deleted again just as easily.
...
fontART does not go online. Not in the sense of “we promise not to send anything”, but: there is no code in it that could. A search across the whole program for `fetch`, `XMLHttpRequest`, `WebSocket` and `sendBeacon` returns **nothing**. There is no visitor counter, no update check, no crash reporting, no font catalogue being consulted.
...
**This is not open source.** The source code of fontART is not published, and no licence is granted beyond the following: you may use the trial free of charge, privately and commercially, and pass the unmodified `.exe` on to whoever you like. Everything else — decompiling, modifying, reselling — is not permitted.
